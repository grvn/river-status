...
Register-ArgumentCompleter -Native -CommandName 'river-status' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'river-status' {
            ...
            [CompletionResult]::new('--show-outputs', '--show-outputs', [CompletionResultType]::ParameterName, 'Explicitly print outputs even if no other flag connected to outputs is in use.')
            [CompletionResult]::new('--show-seat', '--show-seat', [CompletionResultType]::ParameterName, 'Explicitly print seat even if no other flag connected to seat is in use.')
